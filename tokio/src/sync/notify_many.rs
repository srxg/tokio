...
impl AtomicNotification {
    ...
    fn store_release(&self) {
        self.0.store(NOTIFICATION_ALL, Release);
    }
    ...
    fn drop(&mut self) {
        ...
        if !self.is_empty {
            ...
            while let Some(waiter) = self.list.pop_back() {
                ...
                waiter.notification.store_release();
            }
        }
    }
}
...
pub struct Notified<'a> {
    /// The `NotifyMany` being received on.
    notify: &'a NotifyMany,
...
impl UnwindSafe for NotifyMany {}
impl RefUnwindSafe for NotifyMany {}
