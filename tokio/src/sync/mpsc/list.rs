    ...
    fn try_peeking_ahead(&self) -> NonNull<Block<T>> {
        ...
        let mut advance_if_final = |index: usize| {
            if unsafe { next_head.as_ref().is_final() } {
                // Would be nicer to: return res.map_break(|_advanced_to_next_block| ());
                // but, unfortunately, this is an unstable library feature ('control_flow_enum': new API)
                // see: https://github.com/rust-lang/rust/issues/75744
                match Self::advance_step(&mut next_head, index) {
